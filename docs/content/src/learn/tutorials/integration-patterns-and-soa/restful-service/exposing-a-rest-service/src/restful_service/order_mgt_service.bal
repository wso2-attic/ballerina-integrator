    ...
    resource function updateOrder(http:Caller caller, http:Request req, string orderId) {
        ...
        if (updatedOrder is json) {
            ...
            if (existingOrder.length() > 0) {
                map<json> orderJson = <map<json>> existingOrder["Order"];
                map<json> updatedOrderMap = <map<json>> updatedOrder;
                map<json> updatedOrderJson = <map<json>> updatedOrderMap["Order"];
                orderJson["Name"] = updatedOrderJson["Name"];
                orderJson["Description"] = updatedOrderJson["Description"];
                ordersMap[orderId] = existingOrder;
            } else {
                var errorMsg = "Order : " + orderId.toString() + " cannot be found.";
                existingOrder["error"] = errorMsg;
            }
