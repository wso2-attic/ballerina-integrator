...
function testTravelAgencyService() {
    ...
    if(response is http:Response) {
        ...
        // Flight details
        string expectedFlight = "Airline=Emirates ArrivalDate=12-03-2018 ReturnDate=13-04-2018 From=Colombo To=Changi Price=273";
        var resPayload = response.getJsonPayload();
        if (resPayload is json) {
            // json flight = <json> resPayload.Flight;
            test:assertEquals(resPayload.Flight.toString(), expectedFlight, msg = "Response mismatch!");
            // Hotel details
            string expectedHotel = "HotelName=Elizabeth FromDate=12-03-2018 ToDate=13-04-2018 DistanceToLocation=2";
            // json hotel = <json> resPayload.Hotel;
