...
service mobile_bff_service on httpListener {
    ...
    resource function getUserProfile(http:Caller caller, http:Request req) {
        ...
        map<json>|error unreadMessageListMap = map<json>.constructFrom(unreadMessageList);

        // Aggregate the responses to a JSON
        map<json> profileJsonMap = {};
        if(appointmentListMap is map<json>)
        {
            profileJsonMap["Appointments"] = appointmentListMap["Appointments"];
        }
        
        if(unreadMessageList is map<json>)
        {
            profileJsonMap["Messages"] = unreadMessageList["Messages"];
        }

        if(medicalRecordListMap is map<json>)
        {
            profileJsonMap["MedicalRecords"] = medicalRecordListMap["MedicalRecords"];
        }
        
        //construct json from map<json>
