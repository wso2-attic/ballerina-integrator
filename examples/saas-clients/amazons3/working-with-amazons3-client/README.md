...
## Overview
The Ballerina Amazon S3 client provides connectivity to Amazon Simple Storage Service API. It allows users to interact with Amazon S3 API to create, store and download and use data with other services. 

This example explains how to use the S3 clinet to connect with Amazon S3 instance and perform the following operations:
* Create Bucket
* Create Object
* Get Object
* Delete Object
...
- Obtain the credentials such as Access Key, Secret Access Key (API Secret) from Amazon S3 by following the below steps:
...
- Then open a terminal, navigate to `working-with-amazons3-client`, and run the Ballerina project initializing toolkit.
```
$ ballerina init
```
...
The following code is the completed sample which exposes following services:
...
```bash
$ ballerina build working-with-amazons3-client
```
...
```bash
$ ballerina run working-with-amazons3-client.balx
```
...
```ballerina
   $ ballerina run working-with-amazons3-client.bal
```
...
- Set the responseContentType as application/json to retrive a json object and invoke the following curl request to get the newly created object.
```
...
- Invoke the following curl request to delete above object.
```
...
- Invoke the following curl request to delete above bucket.
```
